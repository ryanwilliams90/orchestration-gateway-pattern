...
def _get_state(request: Request) -> AppState:
    """
    Typed accessor for app state.

    `Starlette.State` is a dynamic namespace; `app.state.gateway` returns
    `Any`, and accessing it before lifespan completes raises `AttributeError`
    with a confusing message. This helper:

    1. Returns a properly typed `AppState` for the type checker.
    2. Fails with a clear error if state is missing (e.g., a request reached
       a handler before the lifespan startup populated it).
    """
    ...
    async def _request_id_and_timing(
        ...
        try:
            try:
                ...
                # If the downstream handler raises before producing a
                # response, build one ourselves so the client still sees the
                # request id. Without this, error responses generated by
                # FastAPI's default exception handlers would lack
                # `x-request-id` and break end-to-end correlation.
                outcome = "exception"
